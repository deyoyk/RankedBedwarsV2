...
const THEME_IMAGE_PATH = path.resolve(process.cwd(), 'src', 'asserts', 'themes', 'lunar.png');
...
function ensureFontsRegistered(): void {
  ...
  try {
    if (fs.existsSync(FONT_PATHS.ADAMCGPRO)) {
      ...
      console.warn('[themes/lunar] Missing font file:', FONT_PATHS.ADAMCGPRO);
    }
    if (fs.existsSync(FONT_PATHS.PoppinsMedium)) {
      registerFont(FONT_PATHS.PoppinsMedium, { family: 'PoppinsMedium' });
    } else {
      console.warn('[themes/lunar] Missing font file:', FONT_PATHS.PoppinsMedium);
    }
    if (fs.existsSync(FONT_PATHS.PoppinsRegular)) {
      registerFont(FONT_PATHS.PoppinsRegular, { family: 'PoppinsRegular' });
    } else {
      console.warn('[themes/lunar] Missing font file:', FONT_PATHS.PoppinsRegular);
    }
    fontsRegistered = true;
  } catch (e) {
    console.warn('[themes/lunar] Failed to register fonts:', e);
  }
}
...
export async function generateLunarThemeImage(
...
export default {
  generate: generateLunarThemeImage,
};