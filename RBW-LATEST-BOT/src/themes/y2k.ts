...
const THEME_IMAGE_PATH = path.resolve(process.cwd(), 'src', 'asserts', 'themes', 'y2k.png');
...
function ensureFontsRegistered(): void {
  ...
  try {
    if (fs.existsSync(FONT_PATHS.ADAMCGPRO)) {
      ...
      console.warn('[themes/y2k] Missing font file:', FONT_PATHS.ADAMCGPRO);
    }
    if (fs.existsSync(FONT_PATHS.PoppinsMedium)) {
      registerFont(FONT_PATHS.PoppinsMedium, { family: 'PoppinsMedium' });
    } else {
      console.warn('[themes/y2k] Missing font file:', FONT_PATHS.PoppinsMedium);
    }
    if (fs.existsSync(FONT_PATHS.PoppinsRegular)) {
      registerFont(FONT_PATHS.PoppinsRegular, { family: 'PoppinsRegular' });
    } else {
      console.warn('[themes/y2k] Missing font file:', FONT_PATHS.PoppinsRegular);
    }
    fontsRegistered = true;
  } catch (e) {
    console.warn('[themes/y2k] Failed to register fonts:', e);
  }
}
...
export async function generateY2kThemeImage(
...
export default {
  generate: generateY2kThemeImage,
};